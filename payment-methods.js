...
export default class PaymentMethodsManager {
    ...
    renderPaymentMethodsList(methods) {
        ...
        container.innerHTML = methods.map(method => `
            <div class="flex items-start gap-3 bg-white px-4 py-3 rounded-lg border ${method.is_active ? 'border-gray-200' : 'border-gray-300 bg-gray-50'} cursor-pointer hover:border-[#FF6B2B] transition-colors" onclick="window.paymentMethodsManager.showEditForm(${JSON.stringify(method).replace(/"/g, '&quot;')})">
                <div class="text-[#1b1510] flex items-center justify-center rounded-lg bg-[#f0f2f4] shrink-0 size-10">
                ...
                <div class="flex flex-col flex-1">
                    <div class="flex items-center gap-2">
                        <p class="text-[#1b1510] text-sm font-semibold leading-normal">${method.bank_name}</p>
                        ${!method.is_active ? '<span class="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded">Nonaktif</span>' : ''}
                    </div>
                    <p class="text-[#1b1510] text-sm font-normal leading-normal">${method.account_number}</p>
                    <p class="text-[#1b1510] text-sm font-normal leading-normal">${method.account_holder}</p>
