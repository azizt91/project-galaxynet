...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (loginLogo && loginAppName) {
        loginLogo.src = settings?.logo_url || 'assets/galaxy.png';
        if (loginAppName.tagName === 'IMG') {
            loginAppName.alt = settings?.app_name || 'GALAXY.NET';
        } else {
            loginAppName.textContent = settings?.app_name || 'GALAXY.NET';
        }
    }
    ...
    if (registerLogo && registerAppName) {
        registerLogo.src = settings?.logo_url || 'assets/galaxy.png';
        if (registerAppName.tagName === 'IMG') {
            registerAppName.alt = settings?.app_name || 'GALAXY.NET';
        } else {
            registerAppName.textContent = settings?.app_name || 'GALAXY.NET';
        }
    }
});
