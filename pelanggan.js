...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function setActiveFilterButton(filterType) {
        ...
        Object.values(filterButtons).forEach(btn => {
            btn.classList.remove('bg-[#FF6B2B]');
            btn.classList.add('bg-[#f3ece8]');
            const btnText = btn.querySelector('p');
            if (btnText) {
                btnText.classList.remove('text-white');
                btnText.classList.add('text-[#1b1510]');
            }
        });
        ...
        if (activeButton) {
            activeButton.classList.remove('bg-[#f3ece8]');
            activeButton.classList.add('bg-[#FF6B2B]');
            const activeButtonText = activeButton.querySelector('p');
            if (activeButtonText) {
                activeButtonText.classList.remove('text-[#1b1510]');
    ...
    function initializeEventListeners() {
        ...
        Object.keys(filterButtons).forEach(key => {
            filterButtons[key].addEventListener('click', () => {
                ...
                Object.values(filterButtons).forEach(btn => {
                    btn.classList.remove('bg-[#FF6B2B]', 'text-white'); // Hapus kelas aktif
                    btn.classList.add('bg-[#f3ece8]'); // Kembalikan warna latar non-aktif
                    ...
                    if (textElement) {
                        ...
                        textElement.classList.add('text-[#1b1510]'); // Kembalikan warna teks non-aktif
                    }
                });
            ...
            customerItem.innerHTML = `
                <div class="flex items-center gap-4 w-full">
                    ...
                    <div class="flex flex-col justify-center overflow-hidden">
                        <p class="text-[#1b1510] text-base font-medium truncate">${profile.full_name}</p>
                ...
                unpaidBillsList.innerHTML += `
                    <div class="flex items-center gap-4 bg-[#f0eff3] px-4 min-h-[72px] py-2 justify-between rounded-lg mb-2">
                        <div class="flex flex-col justify-center">
                            <p class="text-[#1b1510] text-base font-medium">${bill.profiles.full_name || '-'}</p>
