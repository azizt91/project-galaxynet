    ...
    function updateTable() {
        ...
        combinedData.forEach((item, index) => {
            ...
            const statusBadge = item.type === 'expense' ? 
                '<span class="px-2 py-1 bg-orange-100 text-purple-800 text-xs font-semibold rounded-full">Pengeluaran</span>' : 
                getStatusBadge(item.status);

            const displayDate = new Date(item.date).toLocaleDateString('id-ID');
            const amountText = item.type === 'expense' ? 
                `<span class="text-orange-600">-${formatter.format(item.amount)}</span>` : 
